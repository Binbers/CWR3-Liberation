...
while {true} do {
    ...
    player setVariable ["KPLIB_hasDirectAccess", (getPlayerUID player) in KP_liberation_commander_actions || {player == ([] call KPLIB_fnc_getCommander)}];
	player setVariable ["KPLIB_isLogistics", (getPlayerUID player) in KP_liberation_commander_actions || {player == ([] call KPLIB_fnc_getCommander)} || {player getUnitTrait "UAVHacker"}];
